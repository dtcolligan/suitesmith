...
The tests in `tests/` are the held-out acceptance battery: the rebuild
is done when they pass against Dom's code. Nothing trained yet. This
README will never claim otherwise.

## Layout (prime-envs convention, exact)

Matches `prime-envs/environments/*/*` file-for-file
(`README.md + pyproject.toml + <pkg>/{__init__.py, taskset.py, verify.py}`):

- `suitesmith/taskset.py` — task construction: family templates (specs,
  references, mutation menus, twins), instance assembly, portfolio
  selection, witness enforcement, prompt templates, train/eval tiers
  (SPEC Q1/Q3/Q4/Q5-ext)
- `suitesmith/verify.py` — verification: runner (sandboxed execution) +
  grader (gates and the Q6 reward policy), kept separate internally for
  interpretability
...
The design document (single source of truth, formerly `SPEC.md`; v1's
